...
///////////////////////////////////////////////////////////////////
  ...
  for (edm::View<pat::Photon>::const_iterator iPho = OOTPhotonsH->begin(); iPho != OOTPhotonsH->end(); ++iPho) {
    ...
    ophoSeedIEta_      .push_back(iPho->iEta());
    ophoSeedIPhi_      .push_back(iPho->iPhi());
...
/*
    ...
    EcalRecHitCollection::const_iterator it = rechitsCollectionEB_->find(seed);
        ...
        {
            EBDetId dit    = it->detid();
            ophoSeedIEta_  .push_back((*it).ieta());
            ophoSeedIPhi_  .push_back((*it).iphi());
        }
        else
        {
            ophoSeedIEta_  .push_back(-99.);
  ...
  for (edm::View<pat::Photon>::const_iterator iPho = photonHandle->begin(); iPho != photonHandle->end(); ++iPho) {
    ...
    /*
        ...
        {
            EBDetId dit   = it->detid();
            phoSeedIEta_  .push_back((*it).ieta());
            phoSeedIPhi_  .push_back((*it).iphi());
        }
        else
        {
            phoSeedIEta_  .push_back(-99.);
...
//add array back and not assign nallcellsEB < 30000 -> failed!
//declared crys [nAllCellsEB] not [30000], no end loop while nAllCellsEB < 30000 -> not compiled


//create another void branch and fill for crystals
//remove allcellscluster, frac, time
//nAllCells initially != 0 (commented out = 0)

//solution -> modifiy the [592] in header file to [30000].

//4/14/2020 phoseedieta -> vector. fill as vector does (phoseed)
