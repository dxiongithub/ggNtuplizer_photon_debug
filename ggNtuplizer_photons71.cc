...
///////////////////////////////////////////////////////////////////
  ...
  for (edm::View<pat::Photon>::const_iterator oPho = OOTPhotonsH->begin(); oPho != OOTPhotonsH->end(); ++oPho) {
    ...
    //phoCalibE_        .push_back(oPho->userFloat("ecalEnergyPostCorr"));
    ophoEt_            .push_back(oPho->et());
    //phoCalibEt_       .push_back(oPho->et()*oPho->userFloat("ecalEnergyPostCorr")/oPho->energy());
    //phoSigmaE_        .push_back(oPho->userFloat("ecalEnergyErrPostCorr"));
    ophoEta_           .push_back(oPho->eta());
    ophoPhi_           .push_back(oPho->phi());
    ophoSCE_           .push_back((*oPho).superCluster()->energy());
    ophoSCRawE_        .push_back((*oPho).superCluster()->rawEnergy());
    ophoESEnP1_        .push_back((*oPho).superCluster()->preshowerEnergyPlane1());
    ophoESEnP2_        .push_back((*oPho).superCluster()->preshowerEnergyPlane2());
    ...
    ophoSCBrem_        .push_back((*oPho).superCluster()->phiWidth()/(*oPho).superCluster()->etaWidth());
    ophohasPixelSeed_  .push_back((Int_t)oPho->hasPixelSeed());
    ophoEleVeto_       .push_back((Int_t)oPho->passElectronVeto());
    ophoR9_            .push_back(oPho->r9());
    ophoHoverE_        .push_back(oPho->hadTowOverEm());
    ophoESEffSigmaRR_  .push_back(lazyTool.eseffsirir(*((*oPho).superCluster())));

    /*
    ophoPFChIso_       .push_back(oPho->userFloat("phoChargedIsolation"));
    ophoPFPhoIso_      .push_back(oPho->userFloat("phoPhotonIsolation"));
    ophoPFNeuIso_      .push_back(oPho->userFloat("phoNeutralHadronIsolation"));
    ophoPFChWorstIso_  .push_back(oPho->userFloat("phoWorstChargedIsolation"));
    ophoIDMVA_         .push_back(oPho->userFloat("PhotonMVAEstimatorRunIIFall17v2Values"));  

    // VID decisions     
    UShort_t tmpphoIDbit = 0;        
    bool isPassLoose  = oPho->photonID("cutBasedPhotonID-Fall17-94X-V2-loose");
    if (isPassLoose)  setbit(tmpphoIDbit, 0);   
    bool isPassMedium = oPho->photonID("cutBasedPhotonID-Fall17-94X-V2-medium");
    if (isPassMedium) setbit(tmpphoIDbit, 1);    
    bool isPassTight  = oPho->photonID("cutBasedPhotonID-Fall17-94X-V2-tight");
    ...
    phoScale_stat_up_.push_back(oPho->userFloat("energyScaleStatUp"));
    phoScale_stat_dn_.push_back(oPho->userFloat("energyScaleStatDown"));
    phoScale_syst_up_.push_back(oPho->userFloat("energyScaleSystUp"));
    phoScale_syst_dn_.push_back(oPho->userFloat("energyScaleSystDown"));
    phoScale_gain_up_.push_back(oPho->userFloat("energyScaleGainUp"));
    phoScale_gain_dn_.push_back(oPho->userFloat("energyScaleGainDown"));
    phoResol_rho_up_ .push_back(oPho->userFloat("energySigmaRhoUp"));
    phoResol_rho_dn_ .push_back(oPho->userFloat("energySigmaRhoDown"));
    phoResol_phi_up_ .push_back(oPho->userFloat("energySigmaPhiUp"));
    phoResol_phi_dn_ .push_back(oPho->userFloat("energySigmaPhiDown"));
    */
    ...
    EcalRecHitCollection::const_iterator ot = rechitsCollectionEB_->find(seed);

        if (ot != rechitsCollectionEB_->end())
        {
            EBDetId dit    = ot->detid();
    ...
    ophoFiredSingleTrgs_     .push_back(matchSinglePhotonTriggerFilters(oPho->et(), oPho->eta(), oPho->phi()));
    ophoFiredDoubleTrgs_     .push_back(matchDoublePhotonTriggerFilters(oPho->et(), oPho->eta(), oPho->phi()));
    ophoFiredTripleTrgs_     .push_back(matchTriplePhotonTriggerFilters(oPho->et(), oPho->eta(), oPho->phi()));
    ophoFiredL1Trgs_         .push_back(matchL1TriggerFilters(oPho->et(), oPho->eta(), oPho->phi()));

    std::vector<float> vCov = lazyToolnoZS.localCovariances( *((*oPho).superCluster()->seed()) );
    ...
    ophoE2x2Full5x5_          .push_back(lazyToolnoZS.e2x2(*((*oPho).superCluster()->seed())));
    ophoE5x5Full5x5_          .push_back(oPho->full5x5_e5x5());
    ophoR9Full5x5_            .push_back(oPho->full5x5_r9());

    //ophoSeedBCE_        .push_back((*oPho).superCluster()->seed()->energy());
    //ophoSeedBCEta_      .push_back((*oPho).superCluster()->seed()->eta());
    
    //ophoSeedTimeFull5x5_.push_back(lazyToolnoZS.SuperClusterSeedTime(*((*oPho).superCluster())));
    ophoMIPChi2_        .push_back(oPho->mipChi2());
    ophoMIPTotEnergy_   .push_back(oPho->mipTotEnergy());
    ophoMIPSlope_       .push_back(oPho->mipSlope());
    ophoMIPIntercept_   .push_back(oPho->mipIntercept());
    ophoMIPNhitCone_    .push_back(oPho->mipNhitCone());
    ophoMIPIsHalo_      .push_back(oPho->mipIsHalo());
